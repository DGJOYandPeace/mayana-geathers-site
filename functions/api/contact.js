 ...
 * POST /api/contact
 * ---------------------------------------------------------------------------
 * One Cloudflare Pages Function serving BOTH site forms, distinguished by the
 * `form_type` field in the payload:
 *
 *   form_type: "contact"    → the Contact page enquiry form
 *   form_type: "newsletter" → the "Receive A Gift!" email signup
 *
 * Email is sent via the Resend API (resend.com) — Cloudflare's currently
 * documented path for sending mail from Pages Functions, after the free
 * MailChannels integration was retired.
 *
 * Required environment variables (set in the Cloudflare Pages dashboard →
 * Settings → Environment variables. NEVER commit these):
 *
 *   RESEND_API_KEY   Secret. From the Resend dashboard → API Keys.
 *   MAIL_FROM        Sender on a Resend-verified domain, e.g.
 *                    "Mayana Geathers Site <notifications@mail.mayanageathers.com>"
...
// Cloudflare routes each method to its own handler and falls back to
// onRequest, so this only ever sees non-POST traffic — answer it clearly
